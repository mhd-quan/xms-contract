...
function readDigit(value: number): string {
  return DIGITS[value] ?? ''
}
...
function readTriple(num: number, full: boolean): string {
  ...
  if (hundred > 0 || full) parts.push(`${readDigit(hundred)} trăm`)
  if (ten > 1) {
    parts.push(`${readDigit(ten)} mươi`)
    if (one === 1) parts.push('mốt')
    else if (one === 5) parts.push('lăm')
    else if (one > 0) parts.push(readDigit(one))
  } else if (ten === 1) {
    parts.push('mười')
