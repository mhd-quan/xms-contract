...
export default function ContractFullrightFormView({ draftId, templateId, documentKind, onBack, onOpenSettings }: Props) {
  ...
  const exportedPath = useDraftStore((state) => state.exportedPath)
  const isDirty = useDraftStore((state) => state.isDirty)
  const isHydrated = useDraftStore((state) => state.isHydrated)
  ...
  const updateField = useDraftStore((state) => state.setField)
  const addStoreRow = useDraftStore((state) => state.addStoreRow)
  ...
  useEffect(() => {
    ...
    const handle = window.setTimeout(() => {
      saveCurrentDraft(draftTitle).catch(() => {
        /* keep dirty state so the next edit retries */
      })
    }, 1200)

    return () => window.clearTimeout(handle)
  }, [draftTitle, isDirty, isHydrated, saveCurrentDraft])
  ...
  async function handleExport() {
    setIsExporting(true)
    try {
      await saveCurrentDraft(draftTitle)
      ...
      if ('finalPath' in saved) {
        await saveCurrentDraft(draftTitle, saved.finalPath)
      }
    } finally {
  ...
  function handleOpenExport() {
    if (exportedPath) osService.openFile(exportedPath)
  }
  ...
  return (
    <div className="form-shell">
      ...
      <div className="form-split">
        ...
        <PreviewPane activeTab={activeTab} onTabChange={setActiveTab} />
      </div>
